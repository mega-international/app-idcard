<template>
  <section class="view-outlet flex flex-col h-full text-light-grey-500 overflow-hidden">
    <header class="sticky top-0 w-full h-12 flex justify-between items-center px-4 bg-dark-blue text-white shadow-md z-50">
      <button class="text-green focus:outline-none" @click="goBack">
        <lit-icon icon="arrow-back" iconset="view"></lit-icon>
      </button>
      <span class="font-bold text-xl truncate">{{ viewOutleTitle }}</span>
      <button class="text-white focus:outline-none" @click="toggleFavorite">
...
export default {
  ...
  computed: {
    ...
    ...mapState({ applications: (state) => state.auth.favoriteApplications }),
    ...mapState({ technologies: (state) => state.auth.favoriteTechnologies }),
    isFavorite() {
